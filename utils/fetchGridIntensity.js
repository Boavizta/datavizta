...
const fetchGridIntensity = async () => {
    ...
    for (let i = 0; i < countries.length; i++) {
        ...
        const countryObject = {
            code: country.code,
            name: countryName,
            gwp_emission_factor: intensityKgs,
            year: year,
        };
    ...
    const csvHeader =
        "country,code,year,gwp_emission_factor,pe_emission_factor,adp_emission_factor,source";
    const csvRows = countriesWithIntensity.map(
        (country) =>

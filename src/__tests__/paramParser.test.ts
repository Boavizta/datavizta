...
describe("Parse parameters", () => {
  test("should parse lifetime", () => {
    expect(
      ParamParser.parseLifetime(new URLSearchParams("lifetime=1"))
    ).toEqual(1);
  });

  test("should parse undefined lifetime", () => {
    expect(ParamParser.parseLifetime(new URLSearchParams(""))).toEqual(
      undefined
    );
  });

  test("should parse yearly", () => {
    expect(ParamParser.parseYearly(new URLSearchParams("yearly=true"))).toEqual(
      true
    );
  });
  ...
  test("should parse undefined region", () => {
    expect(ParamParser.parseRegion(new URLSearchParams(""), [])).toEqual(
      undefined
    );
  });
  ...
  test("should parse region", () => {
    expect(
      ParamParser.parseRegion(new URLSearchParams("region=regionId"), [
        regionItem,
      ])
    ).toEqual(regionItem);
  });
  ...
  test("should parse unknown region", () => {
    expect(
      ParamParser.parseRegion(new URLSearchParams("region=wrongRegion"), [
        regionItem,
      ])
    ).toEqual(undefined);
  });
  ...
  test("should parse mono filter", () => {
    expect(
      ParamParser.parseFilter(new URLSearchParams("category=Datacenter"))
    ).toEqual({
      category: {
        filterType: "text",
        type: "contains",
  ...
  test("should parse multi filter", () => {
    expect(
      ParamParser.parseFilter(
        new URLSearchParams("manufacturer=Lenovo,Seagate")
      )
    ).toEqual({
      manufacturer: {
        filterType: "text",
  ...
  test("should parse multi filters", () => {
    expect(
      ParamParser.parseFilter(
        new URLSearchParams("category=Datacenter&manufacturer=Lenovo,Seagate")
      )
    ).toEqual({
      manufacturer: {
        filterType: "text",
  ...
  test("should parse mono filter", () => {
    expect(
      ParamParser.parseFlatFilter(new URLSearchParams("category=Datacenter"))
    ).toEqual({
      category: ["Datacenter"],
    });
  });
  ...
  test("should build url with region", () => {
    expect(
      ParamParser.buildLink(undefined, regionItem, undefined, null, "false")
    ).toEqual("region=regionId");
  });

  test("should build url with lifetime", () => {
    expect(
      ParamParser.buildLink(1, undefined, undefined, null, "false")
    ).toEqual("lifetime=1");
  });

  test("should build url with yearly", () => {
    expect(
      ParamParser.buildLink(undefined, undefined, true, null, "false")
    ).toEqual("yearly=true");
  });

  test("should build url with no parameter", () => {
    expect(
      ParamParser.buildLink(undefined, undefined, undefined, undefined, "false")
    ).toEqual("");
  });

  test("should build url with with name encoded parameter", () => {
    expect(
      ParamParser.buildLink(
        undefined,
        undefined,
        undefined,
        undefined,
  ...
  test("should build filter models from flatten object", () => {
    expect(
      ParamParser.buildFilterModels({
        manufacturer: ["Lenovo", "Seagate"],
        category: ["Datacenter"],
      })
    ).toEqual(filterModels);
  });

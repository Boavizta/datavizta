<script lang="ts">
    ...
    const filterSubCategoriesDefinition = new Set([
        "Laptop",
        "Monitor",
        "Smartphone",
        "Desktop",
    ...
    const filterManufacturersDefinition = new Set([
        "Apple",
        "Dell",
        "Lenovo",
        "HP",
    ...
    const updateCurrentFilter = (filterName, filterValue, fitlerDefinition) => {
        ...
        if (!(filterName in currentFilterModel)) {
            ...
            const currentFilterValue: Set<String> = new Set(
                currentFilterModel[filterName]
            );
...
<div id="datagrid-root" class="flex flex-wrap justify-between">
    <div class="flex grow flex-wrap my-2 space-x-0.5 > * + *">
        <button
            class="my-2 inline-block blue-button hover:bg-teal-800 disabled:opacity-20 text-white font-bold py-1 px-4 border rounded"
            on:click={resetDataGrid}
            >{$_("datagrid.filter_reset")}
        </button>
            ...
            <div class="inline-flex flex-wrap">
                {#each Array.from(filterManufacturersDefinition) as ManufacturerFilter}
                    <FilterButton
                        ...
                        active={currentFilterModel["manufacturer"] &&
                            currentFilterModel["manufacturer"].includes(
                                ManufacturerFilter
                            )}
                        onButtonClick={() => {
                            updateCurrentFilter(
            ...
            <div class="inline-flex flex-wrap">
                {#each Array.from(filterCategoriesDefinition) as categoryFilter}
                    <FilterButton
                        ...
                        active={currentFilterModel["category"] &&
                            currentFilterModel["category"].includes(
                                categoryFilter
                            )}
                        onButtonClick={() => {
                            updateCurrentFilter(
            ...
            <div class="inline-flex flex-wrap">
                {#each Array.from(filterSubCategoriesDefinition) as subcategoryFilter}
                    <FilterButton
                        ...
                        active={currentFilterModel["subcategory"] &&
                            currentFilterModel["subcategory"].includes(
                                subcategoryFilter
                            )}
                        onButtonClick={() => {

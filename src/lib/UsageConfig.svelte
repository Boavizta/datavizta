<script lang="ts">
    ...
    function change_method(event){
        if(event.detail.value == "Load"){
            cloudConfig.usage.hours_electrical_consumption = null
            updateTimeWorkLoad()
    ...
    function updateTimeWorkLoad(){
        cloudConfig.usage.time_workload = [
                    {
                        time_percentage: 100,
    ...
    function region_select(event){
        cloudConfig.usage.usage_location = event.detail.value
        ...
        <div class="relative col-span-3 w-full mb-2 group">
            ...
            <input bind:value={cloudConfig.usage.years_use_time} type="number" min="1" max="64" class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"/>
        </div>
        <div class="relative col-span-3 w-full mb-2 group">
        ...
        <div class="block relative col-span-3 w-full mb-2 group" id="model">

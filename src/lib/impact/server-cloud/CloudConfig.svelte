<script lang="ts">
    ...
    onMount(async () => {
        ...
        cloudConfig.instance_type = await getfirstitem(
            cloud_instances_route + "?provider=" + cloudConfig.provider
        );
    });
...
<div class="relative min-w-[100px] w-full mb-2 group">
    <label class="block text-sm font-medium text-gray-900"
        >{$_("cloud-config.provider")}</label
    >
    <div style="--borderRadius: 0.5em;">
        <Select
            items={cloud_providers}
            on:select={provider_select}
...
<div class="relative min-w-[100px] w-full mb-2 group">
    <label class="block text-sm font-medium text-gray-900"
        >{$_("cloud-config.instance_type")}</label
    >
    <div style="--borderRadius: 0.5em;">
        <Select
            items={cloud_instances}
            on:select={instance_select}
            value={cloudConfig.instance_type}
        />
    </div>
</div>

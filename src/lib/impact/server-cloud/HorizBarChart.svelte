<script lang="ts">
    ...
    function updateChart(result) {
        result_fab =
            result.embedded.assembly +
            result.embedded.cpu +
            result.embedded.hdd +
            result.embedded.motherboard +
            result.embedded.power_supply +
            result.embedded.ram +
            result.embedded.ssd +
            result.embedded.case;
...
<div class="stacked-bar-graph rounded-bottom">
    <span style="width:{use_pct}%" class="bar-2" />
    <span style="width:{ram_pct}%" class="bar-3" />
    <span style="width:{cpu_pct}%" class="bar-4" />
    <span style="width:{ssd_pct}%" class="bar-5" />
    <span style="width:{hdd_pct}%" class="bar-6" />
    <span style="width:{others_pct}%" class="bar-7" />
</div>
...
<div class="px-1 pb-4 text-xs w-full">
    <span class="bar-3 bar-legend" />
    {$_("impacts.Manufacture")} RAM : {result.embedded.ram.toFixed(nb_rounded)}
    <span class="bar-4 bar-legend ml-2" />
    {$_("impacts.Manufacture")} CPU : {result.embedded.cpu.toFixed(nb_rounded)}
    <span class="bar-5 bar-legend ml-2" />
    {$_("impacts.Manufacture")} SSD : {result.embedded.ssd.toFixed(nb_rounded)}
    <span class="bar-6 bar-legend ml-2" />
    {$_("impacts.Manufacture")} HDD : {result.embedded.hdd.toFixed(nb_rounded)}
    <span class="bar-7 bar-legend ml-2" />
    {$_("impacts.Manufacture")}
    {$_("impacts.others")} : {result_others.toFixed(nb_rounded)}
</div>

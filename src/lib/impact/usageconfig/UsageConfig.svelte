<script lang="ts">
    ...
    function changeExtUsageTime(event, i: Integer) {
        ...
        if (
            parseInt(usageConfig.time_workload[b].time_percentage) +
                parseInt(event.target.value) <=
            100
        ) {
            usageConfig.time_workload[i].time_percentage = event.target.value;
            usageConfig.time_workload[2].time_percentage =
                100 -
                usageConfig.time_workload[0].time_percentage -
                usageConfig.time_workload[1].time_percentage;
            updateExtTimeWorkLoad();
        } else {
...
<div class="relative col-span-3 w-full mb-2 group">
    <label class="block text-sm font-medium text-gray-900"
        >{$_("usage-config.localisation")}</label
    >
...
<div class="relative col-span-3 w-full mb-2 group">
    <label class="block text-sm font-medium text-gray-900"
        >{$_("usage-config.lifespan")}</label
    >
    <input
        bind:value={usageConfig.years_life_time.value}
        on:change={changeLifeTime}
        type="number"
        min="1"
        max="15"
            ...
            <div style="--borderRadius: 0.5em;">
                <Select
                    items={[
                        {
                            value: "Electricity",
                            label: $_("usage-methods.elec"),
                        },
                        { value: "Load", label: $_("usage-methods.load") },
                        {
                            value: "ExtLoad",
                            label: $_("usage-methods.extload"),
                        },
                    ]}
                    on:select={change_method}
                    value={$_("usage-methods.elec")}
                />
            </div>
        {:else}
            <label class="block text-sm font-medium text-gray-900"
                >{$_("usage-config.hoursperday")}</label
            >
            <div style="--borderRadius: 0.5em;">
                <input
                    bind:value={usageConfig.use_time_ratio.hours_per_day}
                    on:change={changeTimeRatio}
                    type="number"
                    min="0"
                    max="24"
                    class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
    ...
    {:else}
        ...
        <div style="--borderRadius: 0.5em;">
            <Select
                items={[
                    { value: "Load", label: $_("usage-methods.load") },
                    { value: "ExtLoad", label: $_("usage-methods.extload") },
                ]}
                on:select={change_method}
                value={$_("usage-methods.load")}
            />
        </div>
    {/if}
</div>
{#if usageType != "Cloud"}
    <div class="relative w-full col-span-3 mb-2 group" id="elec">
        <label class="block text-sm font-medium text-gray-900"
            >{$_("usage-config.avrg_elec")}</label
        >
        <input
            bind:value={usageConfig.avg_power.value}
            on:change={changeElecCons}
            type="number"
            min={usageConfig.avg_power.min}
            max={usageConfig.avg_power.max}
            class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
        />
    </div>
    {#if usageType != "Terminal"}
        <div class="hidden relative col-span-3 w-full mb-2 group" id="load">
            <label class="block text-sm font-medium text-gray-900"
                >{$_("usage-config.load")}</label
            >
            <input
                bind:value={usageConfig.time_workload[0].load_percentage}
                on:change={(event) => {
                    changeUsageLoad(event, 0, false);
                }}
                type="number"
                min="0"
                max="100"
...
{#if usageType != "Terminal"}
    <div id="extload" class="hidden col-span-6 grid grid-cols-6 gap-2">
        <div
            class="p-1 border border-gray-300 rounded-lg relative col-span-2 group"
        >
            <p class="text-center">{$_("usage-methods.timeslot")} 1/3</p>
            <label class="mx-2 block text-xs font-medium text-gray-900"
                >{$_("usage-methods.time")} (%)</label
            >
            <input
                bind:value={usageConfig.time_workload[0].time_percentage}
                on:change={(event) => {
                    changeExtUsageTime(event, 0);
                }}
                type="number"
                min="0"
                max="100"
                class="mx-1 border border-gray-300 text-gray-900 text-xs rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-11/12 p-2"
            />
            <label class="mx-2 block text-xs font-medium text-gray-900"
                >{$_("usage-methods.load")} (%)</label
            >
            <input
                bind:value={usageConfig.time_workload[0].load_percentage}
                on:change={(event) => {
                    changeUsageLoad(event, 0, true);
                }}
                type="number"
                min="0"
                max="100"
        ...
        <div
            class="p-1 border border-gray-300 rounded-lg relative col-span-2 mb-2 group"
        >
            ...
            >
            <input
                bind:value={usageConfig.time_workload[1].time_percentage}
                on:change={(event) => {
                    changeExtUsageTime(event, 1);
                }}
                type="number"
                min="0"
                max="100"
            ...
            >
            <input
                bind:value={usageConfig.time_workload[1].load_percentage}
                on:change={(event) => {
                    changeUsageLoad(event, 1, true);
                }}
                type="number"
                min="0"
                max="100"
        ...
        >
            ...
            <input
                disabled
                bind:value={usageConfig.time_workload[2].time_percentage}
                type="number"
                min="0"
                max="100"
                class="mx-1 border border-gray-300 text-gray-900 text-xs rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-11/12 p-2"
            ...
            >
            <input
                bind:value={usageConfig.time_workload[2].load_percentage}
                on:change={(event) => {
                    changeUsageLoad(event, 2, true);
                }}
                type="number"
                min="0"
                max="100"
...
{#if usageType == "Terminal"}
    <div class="relative col-span-3 w-full mb-2 group">
        <label class="block text-sm font-medium text-gray-900"
            >{$_("usage-config.extendlifespan")}</label
        >
        <input
            bind:value={usageConfig.extendLifetime.value}
            type="number"
            min="0"
            max="5"
            class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"

<script lang="ts">
    ...
    let archetypes = [];
    let items = [
        { value: "terminal", label: $_("terminal-config.terminals") },
        { value: "peripheral", label: $_("terminal-config.peripherals") },
    ];
    let category = {
        value: "terminal",
        label: $_("terminal-config.terminals"),
    };
    ...
    async function updateDefaultUsageValues(category, subcategory, archetype) {
        let temp = await getUsageDefaultValues(
            category,
            subcategory,
            archetype
        );
        ...
        usageConfig.years_life_time.value = Math.round(
            temp["USAGE"]["hours_life_time"]["default"] / 365 / 24
        );
        usageConfig.use_time_ratio.value =
            temp["USAGE"]["use_time_ratio"]["default"];
        usageConfig.use_time_ratio.hours_per_day =
            Math.round(24 * usageConfig.use_time_ratio.value * 10) / 10;
        userDeviceConfig = {
    ...
    onMount(async () => {
        ...
        device_types = Object.keys(
            await getDeviceTypes(userDeviceConfig.category)
        );
        userDeviceConfig.subcategory = device_types[0];
        archetypes = await getArchetypes(
            userDeviceConfig.category,
            userDeviceConfig.subcategory
        );
        userDeviceConfig.archetype = archetypes[0].value;
        updateDefaultUsageValues(
            userDeviceConfig.category,
            userDeviceConfig.subcategory,
            userDeviceConfig.archetype
        );
    });
...
<div class="relative min-w-[100px] w-full mb-2 group">
    <label class="block text-sm font-medium text-gray-900"
        >{$_("terminal-config.category")}</label
    >
    <div style="--borderRadius: 0.5em;">
        <Select {items} on:select={category_select} value={category.label} />
    </div>
</div>
<div class="relative min-w-[100px] w-full mb-2 group">
    <label class="block text-sm font-medium text-gray-900"
        >{$_("terminal-config.subcategory")}</label
    >
    <div style="--borderRadius: 0.5em;">
        <Select
            items={device_types}
            on:select={device_type_select}
            value={userDeviceConfig.subcategory}
        />
    </div>
</div>
<div class="relative min-w-[100px] w-full mb-2 group">
    <label class="block text-sm font-medium text-gray-900"
        >{$_("terminal-config.archetype")}</label
    >
    <div style="--borderRadius: 0.5em;">
        <Select
            items={archetypes}
            on:select={archetype_select}
            value={userDeviceConfig.archetype}
        />
    </div>
</div>

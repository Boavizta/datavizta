<script lang="ts">
    ...
    function updateChart(result, avoided) {
        ...
        fab_rest_pct =
            (Math.max(result.embedded.value - avoided, 0) * 100) / result_total;
        use_rest_pct =
            (Math.max(
                0,
                result_total - Math.max(avoided, result.embedded.value)
            ) *
                100) /
            result_total;
    }
...
{#if result.use.value != undefined && result.embedded.value != undefined}
    ...
    <div class="stacked-bar-graph-small rounded-bottom">
        <span style="width:{use_rest_pct}%" class="bar-2" />
        <span style="width:{fab_rest_pct}%" class="bar-1" />
        <span style="width:{avoided_pct}%" class="bar-3" />
    </div>
    <div class="px-1 text-xs w-full">
        <span class="bar-2 bar-legend" />
        {$_("impacts.Usage")} : {result.use.value.toFixed(nb_rounded)}
        {#if result.use.warnings != undefined}({result.use.warnings}){/if}
        <span class="bar-1 bar-legend ml-2" />
        {$_("impacts.Manufacture")} : {result.embedded.value.toFixed(
            nb_rounded
        )}
        {#if result.embedded.warnings != undefined}({result.embedded
                .warnings}){/if}
        {#if avoided > 0}<span class="bar-3 bar-legend" />
            {$_("impacts.Avoided")} : {avoided.toFixed(nb_rounded)}
            {$_("impacts.Avoideddisclaim")}{/if}
    </div>
{/if}

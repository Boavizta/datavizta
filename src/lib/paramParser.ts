...
export function buildFilterModels(
  flattenFilterModel: FlatFilterModel
): FilterModel {
  const output = {};
...
export function buildLink(
  lifetime,
  selectedRegion: RegionPickerItem,
  isYearly: boolean,
  filterModels: FlatFilterModel,
...
export function parseLifetime(params: URLSearchParams): number {
  return params.get(PARAM_LIFETIME)
    ? Number(params.get(PARAM_LIFETIME))
    : undefined;
}

<script lang="ts">
    ...
    function onUpdateImpacts(
        ...
        ratioScope = Scope.calculateImpacts(
            selectedRows,
            yearly,
            lifetime,
    ...
    function onDataGridUpdate(e) {
        ...
        selectedRows.forEach((r) => {
            selectedSubCategories.add(r.subcategory);
        });
        ...
        selectedRows.forEach((r) => {
            selectedManufacturers.add(r.manufacturer);
        });
    ...
    function disableCustomValues(selectedRows): boolean {
        return (
            ...
            (selectedRows[0]["yearly_tec"] == undefined ||
                selectedRows[0]["gwp_use_ratio"] == undefined)
        );
    }
    ...
    onMount(async () => {
        ...
        lifetime = ParamParser.parseLifetime(
...
<div class="flex flex-col">
    <div class="flex flex-row flex-wrap md:mt-2 justify-around">
        ...
        <div class="flex flex-row flex-wrap-reverse justify-center">
            <div
                id="viz-container"
                class="flex flex-col md:rounded-l content-center py-5 px-10 border-solid border-2 border-teal-500/20"
            >
                {#if isNaN(ratioScope.total)}
                    <div
                        id="result-title"
                        class="text-xl font-normal text-center"
                    >
                        {$_("pie.title")}
                    </div>
                    <div
                        id="result-highlight"
                        class="text-center text-4xl font-medium my-2 text-green"
                    >
                        {impactTotal} kgCO2eq
                    </div>
                    <div
                        id="result-impossible"
                        class="max-w-sm text-m font-normal text-center"
                    >
                        {$_("pie.impossible")}
                    </div>
                {:else}
                    ...
                    <div
                        id="result-subtitle"
                        class="text-sm font-light text-center text-gray-800 pl-2"
                    >
                        {#if selectedRows.length === 1}
                            {$_("pie.subtitle_unique_equipment", {
                                values: {
                                    total: ratioScope.total,
                                    name:
                                        selectedRows[0].manufacturer +
                                        " " +
                                        selectedRows[0].name,
                                },
                            })}
                        {:else if selectedSubCategories.size < 3}
                            {$_(
                                "pie.subtitle_multiple_equipment_categories_details",
                                {
                                    values: {
                                        number: Math.min(
                                            ratioScope.scope3.lines,
                                            ratioScope.scope2.lines
                                        ),
                        ...
                        {:else if selectedCategories.size < 3}
                            {$_(
                                "pie.subtitle_multiple_equipment_types_details",
                                {
                                    values: {
                                        number: Math.min(
                                            ratioScope.scope3.lines,
                        ...
                        {:else}
                            {$_("pie.subtitle_multiple_equipment_categories", {
                                values: {
                                    number: Math.min(
                                        ratioScope.scope3.lines,
                                        ratioScope.scope2.lines
                    ...
                    <div
                        id="result-subtitle"
                        class="text-sm font-light text-center text-gray-600 pl-2"
                    >
                        {#if !isDefaultRegion}
                        ...
                        {#if lifetime}
                            {!isDefaultRegion ? " / " : ""}
                            {$_("pie.lifetime")}: {lifetime}
                            {$_("pie.year(s)")}
                        {:else if !isNaN(medianlifetime)}
                            {!isDefaultRegion ? " / " : ""}
                            {$_("pie.medianlifetime")}: {medianlifetime}
                            {$_("pie.year(s)")}
                        {/if}
                    </div>
            ...
            <div
                id="form-container"
                class="flex flex-col md:rounded-r px-5 py-5 bg-opacity-20 max-w-sm bg-teal-500"
            >
                <div id="title" class="text-xl mb-5 font-medium text-center">
                    {$_("manufdata.custom_values")}
                </div>
                    ...
                    <div class="flex">
                        ...
                        {#key $locale}
                            <RegionPicker
                                ...
                                bind:isDefaultRegion
                                isDisabled={disabledCustomValue}
                            />
                        {/key}
                    </div>
                    <small
                        ...
                        >{$_(
                            "manufdata.select_country_elec_impact_tooltip"
                        )}</small
                    >
                </div>
                ...
                <div class="mb-3">
                    ...
                    <div class="flex">
                        <input
                            id="lifetime"
                            bind:value={lifetime}
                            on:input={changeLifetime}
                            type="number"
                            class="border-2 pl-2 w-auto"
                            min="0.5"
                            max="100"
                            step="0.5"
                            disabled={disabledCustomValue}
                        />
                        <span
                            class="text-sm border-2 rounded-r px-4 py-1 bg-gray-300 whitespace-no-wrap"
                        >
                            {$_("manufdata.years")}
                        </span>
                    </div>
                    <small
                        id="lifetimeHelp"
                        class="block mt-1 text-xs text-gray-600"
                        >{$_("manufdata.years_tooltip")}</small
                    >
                </div>
                ...
                <div class="flex-row mx-auto">
                    <div
                        id="title export"
                        class="text-xl mt-3 font-medium text-center"
                    >
                        {$_("pie.export")}
                    </div>
                ...
                <div class="flex-row mx-auto">
                    ...
                    <ShareLinkButton
                        ...
                        {yearly}
                        {filterModels}
                        singleItemSelected={singleItemSelected(selectedRows)}
                    />
                </div>
            </div>
        </div>

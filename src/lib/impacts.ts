...
export function calculateImpacts(
  ...
  yearly: boolean,
  lifetime: number,
  electricalImpactFactor: number
) {
  const scope2 = impactScope2(
    selectedRows,
    yearly,
    lifetime,
    electricalImpactFactor
  );
...
export function medianlifetime(rows_selection: Row[]) {
  ...
  const medianlife =
    Math.round((manuflifetime * 100) / lineswithlifetime) / 100;
  return medianlife;
}

/* calculate scope 3 impacts */
export function impactScope3(
  rows_selection: Row[],
  yearly: boolean,
  lifetime: number
): ScopeResult {
...
export function impactScope2(
  ...
  rows_selection.forEach((row) => {
    ...
    if (yearly == true) {
      if (
        lifetime == undefined &&
        electricalImpactFactor == -1 &&
        rowlifetime != undefined
      ) {
        if (
          row["gwp_total"] != undefined &&
      ...
      if (lifetime == undefined && electricalImpactFactor == -1) {
        ...
        if (
          ...
          scope2 +=
            ((row["gwp_total"] * row["gwp_use_ratio"]) / rowlifetime) *
            lifetime;
          processedLines++;
        }
      } else if (lifetime == undefined && electricalImpactFactor !== -1) {
        if (row["yearly_tec"] != undefined) {
          scope2 +=
            row["yearly_tec"] * row["lifetime"] * electricalImpactFactor;
          processedLines++;
        }
      } else if (lifetime != undefined && electricalImpactFactor !== -1) {
        if (row["yearly_tec"] != undefined) {
...
/* calculate scope 2 impacts */
export function impactScope2byRow(
  row: Row,
  lifetime: number,
  electricalImpactFactor: number
) {
  ...
  if (lifetime == undefined && electricalImpactFactor == -1) {
    ...
    if (
      ...
      scope2 +=
        ((row["gwp_total"] * row["gwp_use_ratio"]) / row["lifetime"]) *
        lifetime;
    }
  } else if (lifetime == undefined && electricalImpactFactor !== -1) {
    if (row["yearly_tec"] != undefined) {
